...
import { initializeEcology, evolveEcology, ecologyBudget } from './ecology.js';
...
function addEvent(w, details) {
  const event = {
    ...
    ...(details.evidence ? { evidence: clone(details.evidence) } : {}),
  };
  w.events.push(event);
...
export function createWorld(options = {}) {
  ...
  structure(w, 'k-hollow-hall').formerKind = 'home';
...
export function getEntity(w, id) {
  ...
  return [...w.regions, ...w.settlements, ...w.characters, ...w.routes, ...w.cultures, ...w.institutions, ...w.settlements.flatMap(s => s.structures), ...w.events, ...w.threads, ...(w.ecology?.sites || []), ...(w.power ? [w.power] : [])].find(e => e.id === id) || null;
}
...
function resolveSeeds(w) {
  ...
  if (chosen === 'exchange') {
    ...
    addStructure(w, h, { id: 'k-hearth-nursery', name: 'The Living Archive', kind: 'garden', foodProduction: 0.3, x: -105, y: 93, description: 'The endangered seeds were planted here instead of returned to storage.' }, e);
  }
...
function recover(w) {
  ...
  const h = settlement(w, 's-hearth');
  // Planting costs food and materials; the harvest arrives through subsequent
  // ecological days. Keep eight days of meals instead of demanding a rich pantry.
  const plantingFood = 4, meals = ecologyBudget(w, h).foodDemand;
  if (h.food < plantingFood + meals * 8 || h.materials < 16 || h.habitat < 40) return;
  ...
  h.materials = clamp(h.materials - 16); h.food = clamp(h.food - plantingFood); w.flags.gardenEfficiency += shared ? 0.8 : 0.4; w.flags.recovery = true;
  ...
  const e = addEvent(w, { kind: 'seed-recovery', family: 'recovery', category: 'infrastructural', severity: 2, title: shared ? 'Water that remembers the way' : 'Room for the saved seeds', text: recoveryText, entities: ['s-hearth', 'c-senn', 'c-ivo', 'c-nera', 'k-hearth-garden'], causes: [lastEvent(w, 'seed-decision').id, lastEvent(w, 'readers-returned')?.id, lastEvent(w, 'eastern-contact')?.id], observed: [`Hearth materials: ${before.materials} → ${h.materials}.`, `Hearth food: ${before.food} → ${h.food}.`, `Average garden production increased by ${shared ? 0.8 : 0.4} before seasonal variation.`], decision: decision(senn, [`Hearth has ${before.food} food and ${before.materials} materials.`, `Usable garden knowledge is ${h.knowledge}.`, 'Nera has resolved the threatened seed collection.'], [{ id: 'channels', label: 'Build copied capillary channels', available: shared, reason: shared ? 'The community has enough transmitted knowledge.' : 'Nobody here yet knows the required pattern.' }, { id: 'beds', label: 'Build familiar raised beds', available: true, reason: 'Local skills and materials suffice.' }, { id: 'keep', label: living ? 'Leave the nursery as it is' : 'Leave the seeds stored', available: true, reason: 'The immediate threat has passed.' }], outcome, ['Care 87: use preserved seeds to feed people.', 'Curiosity 63: adopt an understood method when available.']) });
  addStructure(w, h, { id: 'k-hearth-recovery', name: shared ? 'The Borrowed Channels' : 'The Second Garden', kind: 'garden', foodProduction: shared ? 0.8 : 0.4, x: -112, y: -50, description: shared ? 'This garden uses knowledge carried from another community.' : w.flags.archiveResolved === 'plant' ? 'This garden extends the living nursery Nera and Senn planted.' : 'This garden began with seeds Nera saved in the dry loft.' }, e);
